...
const RoomPanel = ({
  ...
  return (
    <button
      ...
      className="panel relative p-5 text-left w-full overflow-hidden transition-colors hover:border-odin-accent group"
    >
      {/* Subtle ambient hint — left edge glows when room is active */}
      <div
        className="absolute left-0 top-0 bottom-0 w-[2px] transition-all duration-500"
        style={{
          background: anyOn
            ? "linear-gradient(180deg, hsl(var(--accent)), hsl(var(--accent-dim)))"
            : occupied
              ? "hsl(var(--info))"
              : "transparent",
          boxShadow: anyOn
            ...
            : occupied
              ? "0 0 8px hsl(var(--info) / 0.5)"
              : "none",
          opacity: anyOn ? Math.max(0.4, avgLevel / 100) : occupied ? 0.8 : 0,
        }}
      />
